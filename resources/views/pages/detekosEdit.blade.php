@extends('layouts.app', ['activePage' => 'Deteks', 'titlePage' => __('Ubah Deteks')])
...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('detekos.update',$detekos->id ) }}" autocomplete="off" class="form-horizontal">
            @csrf
            @method('PUT')

            <div class="card ">
              <div class="card-header card-header-primary">
                <h4 class="card-title">{{ __('Ubah Deteksi') }}</h4>
              </div>
              <div class="card-body ">
                @if (session('status'))
                  <div class="row">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    ...
                    <input class="form-control{{ $errors->has('nama') ? ' is-invalid' : '' }}" name="nama" id="input-nama" type="text" placeholder="{{ __('Nama') }}" value="{{$detekos->nama}}" required="true" aria-required="true"/>
                      @if ($errors->has('nama'))
                ...
                <div class="row">
                    ...
                    <div class="col-sm-7">
                      ...
                      <input class="form-control{{ $errors->has('video') ? ' is-invalid' : '' }}" name="video" id="input-video" type="text" placeholder="{{ __('link video') }}" value="{{$detekos->video}}" required="true" aria-required="true"/>
                        @if ($errors->has('video'))
                      ...
                      <div class="form-group{{ $errors->has('jenisEdukasi_id') ? ' has-danger' : '' }}">
                        <select class="form-control" class="form-control" name="jenisEdukasi_id" id="jenisEdukasi_id">
                            ...
                            @foreach ($jenisEdukasi as $jenisEdukasi) {
                                @if($detekos->idjenis==$jenisEdukasi->id)
