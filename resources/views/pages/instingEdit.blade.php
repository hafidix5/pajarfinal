@extends('layouts.app', ['activePage' => 'Insting', 'titlePage' => __('Ubah Insting')])
...
@section('content')
  <div class="content">
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-12">
          <form method="post" action="{{ route('insting.update',$insting->id ) }}" autocomplete="off" class="form-horizontal">
            @csrf
            @method('PUT')
              ...
              <div class="card-body ">
                ...
                <div class="row">
                  ...
                  <div class="col-sm-7">
                    ...
                    <input class="form-control{{ $errors->has('nama') ? ' is-invalid' : '' }}" name="nama" id="input-nama" type="text" placeholder="{{ __('Nama') }}" value="{{$insting->nama}}" required="true" aria-required="true"/>
                      @if ($errors->has('nama'))
                ...
                <div class="row">
                    ...
                    <div class="col-sm-7">
                      ...
                      <input class="form-control{{ $errors->has('video') ? ' is-invalid' : '' }}" name="video" id="input-video" type="text" placeholder="{{ __('link video') }}" value="{{$insting->video}}" required="true" aria-required="true"/>
                        @if ($errors->has('video'))
                      ...
                      <div class="form-group{{ $errors->has('jenisEdukasi_id') ? ' has-danger' : '' }}">
                        <select class="form-control" class="form-control" name="jenisEdukasi_id" id="jenisEdukasi_id">
                            ...
                            @foreach ($jenisEdukasi as $jenisEdukasi) {
                                @if($insting->idjenis==$jenisEdukasi->id)
